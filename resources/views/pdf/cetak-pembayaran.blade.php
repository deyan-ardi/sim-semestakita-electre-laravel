            ...
            <td class="w-50 text-left">
                <table class="w-100">
                    <tr>
                        ...
                        <td>: {{ $rekapan[0]->user->name }}</td>
                    </tr>
                    <tr>
                        <td width="120px">
                            <b>Kode Member</b>
                        </td>
                        <td>: {{ $rekapan[0]->user->no_member }}</td>
                    </tr>
                    <tr>
                        <td width="120px">
                            <b>No WhatsApp</b>
                        </td>
                        <td>: {{ empty($rekapan[0]->user->no_telp) ? '~' : $rekapan[0]->user->no_telp }}</td>
                    </tr>
                    <tr>
                        <td width="120px">
                            <b>Email</b>
                        </td>
                        <td>: {{ $rekapan[0]->user->email }}</td>
                    </tr>
                    <tr>
                        <td width="120px">
                            <b>Jenis Transaksi</b>
            ...
            <td class="w-50 text-right">
                <table class="w-100">
                    ...
                    <tr>
                        ...
                        <td>
                            {{ \Carbon\Carbon::parse($rekapan[0]->created_at)->format('d F Y H:i') . ' WITA' }}
                        </td>
                    </tr>
                    <tr>
                        <td>
        ...
        <table class="table table-sm table-striped" style="width: 100%;">
            ...
            <tbody>
                @php
                    ...
                    $i = 1;
                @endphp
                @foreach ($rekapan as $detailTagihan)
                    <tr>
                        <td style="width:25px; vertical-align:middle;text-align: center;">{{ $i++ }}</td>
                        <td>{{ $detailTagihan->deskripsi }}</td>
                        <td>{{ $detailTagihan->no_tagihan }}</td>
                        <td>{{ $detailTagihan->no_pembayaran }}</td>
                        <td class="text-right">Rp. {{ number_format($detailTagihan->sub_total, 0, ',', '.') }}</td>
                        <td class="text-right">Rp. {{ number_format($detailTagihan->sub_total_denda, 0, ',', '.') }}
                        </td>
                        <td>{{ $detailTagihan->status_denda }}</td>
                        <td class="text-right">Rp. {{ number_format($detailTagihan->total_tagihan, 0, ',', '.') }}
                        </td>
                    </tr>
                    @php
                        $total = $total + $detailTagihan->total_tagihan;
                    @endphp
                @endforeach
                <tr>
