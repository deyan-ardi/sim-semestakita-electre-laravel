...
{{-- Meta --}}
@section('meta-name', 'Pergantian Nomor Rekening')
@section('meta-description', 'Data Pergantian Nomor Rekening')
@section('meta-keyword', 'Pergantian Nomor Rekening')
{{-- End Meta --}}
@section('title', 'Pergantian Nomor Rekening')
            ...
            <div class="col-12 m-b-100">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Validasi Pergantian Rekening</h5>
                    </div>
                    <div class="card-body row justify-content-center">
                        <p class="text-center">Permintaan pergantian nomor rekening menjadi <span
                                class="text-primary">({{ $rekening }} - Bank
                                {{ $bank }}) </span> telah di proses. Silahkan masukkan
                            kode
                            OTP yang telah dikirimkan melalui
                            nomor
                            WhatsApp (<span
                                class="text-primary">{{ $user->no_telp == null ? 'Belum Disetel' : '0' . $user->no_telp }}</span>)
                            dan Email (<span class="text-primary">{{ $user->email }}</span>) untuk memvalidasi</p>
                        <div class="col-6">
                            ...
                            <div class="mb-3">
                                <p class="text-center">Tidak menerima OTP atau OTP tidak bekerja ? <br> <span id="time_data"
                                        class="text-primary"></span></p>
                                <form action="{{ route('enduser.profil.rekening.aksi', [Auth::user()->id]) }}"
                                    method="POST">
                                    @csrf
                                    <input type="hidden" name="bank" value="{{ $bank }}">
                                    <input type="hidden" name="no_rek" value="{{ $rekening }}">
